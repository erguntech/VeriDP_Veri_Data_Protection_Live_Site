<div class="alert bg-light-{{ $data['alert_type'] }} border border-{{ $data['alert_type'] }} d-flex flex-column flex-sm-row p-5">
    <span class="svg-icon svg-icon-2hx svg-icon-{{ $data['alert_type'] }} me-4 d-none d-sm-block">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" style="margin-top: 3px;">
    ...
    <div class="d-flex flex-column pe-0 pe-sm-10"><h5 class="mb-1">@ {{ $data['alert_title'] }}</span></h5><span class="text-{{ $data['alert_type'] }}">- {{ $data['alert_content'] }}</span></div>
</div>
