...
@section('PageTitle', __('messages.academy_contents.presentations.01'))
...
@section('PageContent')
    @if($academyContentPresentations->count() == 0)
                    ...
                    <div class="card-body d-flex flex-column justify-content-between bgi-no-repeat bgi-size-cover bgi-position-x-center pb-0" style="background-position: 100% 50%; background-image:url('assets/media/stock/900x600/42.png')">
                        <div class="alert alert-primary d-flex align-items-center p-5">
                            ...
                            <div class="d-flex flex-column">
                                <h4 class="mb-1">@ {{ __('messages.academy_contents.presentations.alert.01') }}</h4>
                                <span>- {{ __('messages.academy_contents.presentations.alert.02') }}</span>
                                <span>- {{ __('messages.academy_contents.presentations.alert.03') }}</span>
                            </div>
                        </div>
    ...
    @else
        ...
        <div class="row g-6 g-xl-9 mb-6 mb-xl-9">
            @foreach($academyContentPresentations as $academyContentPresentation)
                        ...
                        <div class="card-body d-flex justify-content-center text-center flex-column p-8">
                            <span class="text-gray-800 text-hover-primary d-flex flex-column">
                                ...
                                <div class="fs-5 fw-bold mb-2 document-name">
                                    {{ $academyContentPresentation->academy_content_name }}
                                </div>
                                <div class="fs-5 fw-bold mb-2 mt-2">
                                    <a href="{{ route('AcademyContents.DownloadAcademyContent', $academyContentPresentation->id) }}" type="button" class="btn btn-sm btn-success">
                                        {{ __('messages.academy_contents.presentations.03') }}
                                    </a>
                                </div>
                            </span>
                            <div class="fs-7 fw-semibold text-gray-500">
                                {{ __('messages.academy_contents.presentations.02') }}: {{ date('d/m/Y', strtotime($academyContentPresentation->created_at)) }}
                            </div>
                        </div>
                    </div>
                </div>
