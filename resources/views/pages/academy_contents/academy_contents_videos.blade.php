...
@section('PageTitle', __('messages.academy_contents.videos.01'))
...
@section('PageContent')
    @if($academyContentVideos->count() == 0)
                    ...
                    <div class="card-body d-flex flex-column justify-content-between bgi-no-repeat bgi-size-cover bgi-position-x-center pb-0" style="background-position: 100% 50%; background-image:url('assets/media/stock/900x600/42.png')">
                        <div class="alert alert-primary d-flex align-items-center p-5">
                            ...
                            <div class="d-flex flex-column">
                                <h4 class="mb-1">@ {{ __('messages.academy_contents.videos.alert.01') }}</h4>
                                <span>- {{ __('messages.academy_contents.videos.alert.02') }}</span>
                                <span>- {{ __('messages.academy_contents.videos.alert.03') }}</span>
                            </div>
                        </div>
    ...
    @else
        ...
        <div class="row g-6 g-xl-9 mb-6 mb-xl-9">
            @foreach($academyContentVideos as $academyContentVideo)
                        ...
                        <div class="card-body d-flex justify-content-center text-center flex-column p-8">
                            <span class="text-gray-800 text-hover-primary d-flex flex-column">
                                ...
                                <div class="fs-5 fw-bold mb-2 document-name">
                                    {{ $academyContentVideo->academy_content_name }}
                                </div>
                                <div class="card overlay overflow-hidden mb-2 mt-2">
                                    <div class="card-body p-0">
                                        <div class="overlay-wrapper">
                                            <img src="{{ asset('assets/media/misc/play.jpg') }}" alt="" class="rounded"/>
                                        </div>
                                        <div class="overlay-layer bg-dark bg-opacity-25">
                                            <a href="{{ $academyContentVideo->document_url }}" class="btn btn-primary btn-shadow" target="_blank">{{ __('messages.academy_contents.index.03') }}</a>
                                        </div>
                                    </div>
                                </div>
                            </span>
                            <div class="fs-7 fw-semibold text-gray-500">
                                {{ __('messages.academy_contents.videos.02') }}: {{ date('d/m/Y', strtotime($academyContentVideo->created_at)) }}
                            </div>
                        </div>
                    </div>
                </div>
