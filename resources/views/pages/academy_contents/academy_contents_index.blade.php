...
@section('PageTitle', __('messages.academy_contents.index.01'))
        ...
        <div class="col-xl-12">
            ...
            <div class="card">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-primary-active fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">{{ __('messages.academy_contents.index.02') }} {{ __('messages.datatables.02') }}</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar">
                        <a href="{{ route('AcademyContents.Create') }}" class="btn btn-light-success btn-icon btn-sm">
                ...
                <div class="card-body">
                    ...
                    <table id="datatable" class="table table-hover table-rounded table-row-bordered table-row-gray-200 gy-1 gs-10" style="min-height: 100%; width: 100% !important;">
                        ...
                        <tr class="fw-bolder fs-7 text-uppercase gy-5">
                            ...
                            <th>{{ __('messages.academy_contents.index.datatables.01') }}</th>
                            <th>{{ __('messages.academy_contents.index.datatables.02') }}</th>
                            <th>{{ __('messages.academy_contents.index.datatables.03') }}</th>
...
@section('PageCustomJS')
    <script>
        var table, dt;
        var editUrl = '{{ route("AcademyContents.Edit", ":id") }}';
        ...
        var initDatatable = function () {
            dt = $("#datatable").DataTable({
                ...
                info: false,
                ajax: {
                    url : "{{ route('AcademyContents.Index') }}",
                },
                language: {
                    @if(App::getLocale() == 'tr')
                ...
                columns: [
                    { data: 'id' },
                    { data: 'academy_content_name' },
                    { data: 'academy_content_type' },
                    { data: 'status' },
                    { data: null },
                ],
                columnDefs: [
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "academymanagement/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {
