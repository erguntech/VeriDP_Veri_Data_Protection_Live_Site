...
@section('PageTitle', __('messages.gdpr_adaptation_questions.index.01'))
...
@section('Breadcrumb')
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 ">
        <li class="breadcrumb-item text-muted">
            <a href="{{ route('GDPRAdaptationQuestions.Index') }}" class="text-muted text-hover-primary">{{ Settings::get('app_alias') }}</a>
        </li>
    </ul>
@endsection
        ...
        <div class="col-xl-12">
            ...
            <div class="card">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-primary-active fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">{{ __('messages.gdpr_adaptation_questions.index.02') }} {{ __('messages.datatables.02') }}</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar">
                        <a href="{{ route('GDPRAdaptationQuestions.Create') }}" class="btn btn-light-success btn-icon btn-sm">
                ...
                <div class="card-body">
                    ...
                    <table id="datatable" class="table table-hover table-rounded table-row-bordered table-row-gray-200 gy-1 gs-10" style="min-height: 100%; width: 100% !important;">
                        ...
                        <tr class="fw-bolder fs-7 text-uppercase gy-5">
                            ...
                            <th>{{ __('messages.gdpr_adaptation_questions.index.datatables.01') }}</th>
                            <th>{{ __('messages.gdpr_adaptation_questions.index.datatables.02') }}</th>
                            <th>{{ __('messages.gdpr_adaptation_questions.index.datatables.03') }}</th>
                            <th>{{ __('messages.gdpr_adaptation_questions.index.datatables.04') }}</th>
...
@section('PageCustomJS')
    <script>
        var table, dt;
        var editUrl = '{{ route("GDPRAdaptationQuestions.Edit", ":id") }}';
        ...
        var initDatatable = function () {
            dt = $("#datatable").DataTable({
                ...
                info: false,
                ajax: {
                    url : "{{ route('GDPRAdaptationQuestions.Index') }}",
                },
                language: {
                    @if(App::getLocale() == 'tr')
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "gdpradaptation/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {
