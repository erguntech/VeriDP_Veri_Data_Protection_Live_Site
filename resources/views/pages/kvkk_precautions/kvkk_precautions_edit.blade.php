...
@section('PageTitle', __('messages.kvkk_precautions.edit.01'))
        ...
        <div class="col-xl-12">
            ...
            <div class="card shadow-sm">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">{{ __('messages.kvkk_precautions.edit.02') }}</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar"></div>
                </div>
                <div class="card-body">
                    <form action="{{ route('KVKKPrecautions.Update', $kvkkPrecaution->id) }}" id="editForm" enctype="multipart/form-data" method="POST" autocomplete="off">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-12">
                                <label for="input-title" class="required form-label">{{ __('messages.kvkk_precautions.form.01') }}</label>
                                <input type="text" name="input-title" id="input-title" class="form-control @error('input-title') is-invalid error-input @enderror" placeholder="{{ __('messages.kvkk_precautions.form.01') }} {{ __('messages.forms.01') }}" maxlength="1000" value="{{ old('input-title', $kvkkPrecaution->title) }}"/>
                        ...
                        <div class="row mt-6">
                            <div class="col-12">
                                ...
                                <input type="text" name="input-content" id="input-content" class="form-control @error('input-content') is-invalid error-input @enderror" placeholder="{{ __('messages.kvkk_precautions.form.02') }} {{ __('messages.forms.01') }}" maxlength="1000" value="{{ old('input-content', $kvkkPrecaution->content) }}"/>
                        ...
                        <div class="row mt-6">
                            ...
                            <div class="col-6">
                                ...
                                <select class="form-select @error('input-caution_type') is-invalid error-input @enderror" name="input-caution_type" id="input-caution_type" data-control="select2" data-placeholder="{{ __('messages.forms.06') }}" data-hide-search="true">
                                    <option></option>
                                    <option value="I" {{ old('input-caution_type', $kvkkPrecaution->caution_type) == 'I' ? 'selected' : '' }}>{{ __('messages.kvkk_precautions.form.03.01') }}</option>
                                    <option value="T" {{ old('input-caution_type', $kvkkPrecaution->caution_type) == 'T' ? 'selected' : '' }}>{{ __('messages.kvkk_precautions.form.03.02') }}</option>
                                </select>
                            ...
                            <div class="col-6">
                                ...
                                <select class="form-select @error('input-is_active') is-invalid error-input @enderror" name="input-is_active" id="input-is_active" data-control="select2" data-placeholder="{{ __('messages.forms.06') }}" data-hide-search="true">
                                    <option></option>
                                    <option value="1" {{ old('input-is_active', $kvkkPrecaution->is_active) == '1' ? 'selected' : '' }}>{{ __('messages.kvkk_precautions.form.04.01') }}</option>
                                    <option value="0" {{ old('input-is_active', $kvkkPrecaution->is_active) == '0' ? 'selected' : '' }}>{{ __('messages.kvkk_precautions.form.04.02') }}</option>
                                </select>
                    ...
                    <a href="{{ route('KVKKPrecautions.Index') }}" class="btn btn-light-danger btn-sm ms-2">{{ __('messages.forms.04') }}</a>
                </div>
            </div>
        </div>
    </div>
