...
@section('PageTitle', __('messages.company_departments.edit.01'))
        ...
        <div class="col-xl-12">
            ...
            <div class="card shadow-sm">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">{{ __('messages.company_departments.edit.02') }}</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar"></div>
                </div>
                <div class="card-body">
                    <form action="{{ route('CompanyDepartments.Update', $companyDepartment->id) }}" id="editForm" enctype="multipart/form-data" method="POST" autocomplete="off">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-12">
                                <label for="input-department_name" class="required form-label">{{ __('messages.company_departments.form.01') }}</label>
                                <input type="text" name="input-department_name" id="input-department_name" class="form-control @error('input-department_name') is-invalid error-input @enderror" placeholder="{{ __('messages.company_departments.form.01') }} {{ __('messages.forms.01') }}" maxlength="50" value="{{ old('input-department_name', $companyDepartment->department_name) }}"/>
                        ...
                        <div class="row mt-6">
                            <div class="col-12">
                                ...
                                <select class="form-select @error('input-is_active') is-invalid error-input @enderror" name="input-is_active" id="input-is_active" data-control="select2" data-placeholder="{{ __('messages.forms.06') }}" data-hide-search="true">
                                    <option></option>
                                    <option value="1" {{ old('input-is_active', $companyDepartment->is_active) == '1' ? 'selected' : '' }}>{{ __('messages.company_departments.form.02.01') }}</option>
                                    <option value="0" {{ old('input-is_active', $companyDepartment->is_active) == '0' ? 'selected' : '' }}>{{ __('messages.company_departments.form.02.02') }}</option>
                                </select>
