...
@section('PageTitle', __('messages.company_documents.index.01'))
        ...
        <div class="col-xl-12">
            ...
            <div class="card">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-primary-active fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">{{ __('messages.company_documents.index.02') }} {{ __('messages.datatables.02') }}</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar">
                        <a href="{{ route('CompanyDocuments.Create') }}" class="btn btn-light-success btn-icon btn-sm">
                ...
                <div class="card-body">
                    ...
                    <table id="datatable" class="table table-hover table-rounded table-row-bordered table-row-gray-200 gy-1 gs-10" style="min-height: 100%; width: 100% !important;">
                        ...
                        <tr class="fw-bolder fs-7 text-uppercase gy-5">
                            ...
                            <th>{{ __('messages.company_documents.index.datatables.01') }}</th>
                            <th>{{ __('messages.company_documents.index.datatables.02') }}</th>
                            <th>{{ __('messages.company_documents.index.datatables.03') }}</th>
                            <th>{{ __('messages.company_documents.index.datatables.04') }}</th>
...
@section('PageCustomJS')
    <script>
        var table, dt;
        var editUrl = '{{ route("CompanyDocuments.Edit", ":id") }}';
        ...
        var initDatatable = function () {
            dt = $("#datatable").DataTable({
                ...
                info: false,
                ajax: {
                    url : "{{ route('CompanyDocuments.Index') }}",
                },
                language: {
                    @if(App::getLocale() == 'tr')
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "companydocuments/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {
