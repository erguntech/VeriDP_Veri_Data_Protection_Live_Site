...
@section('PageTitle', __('messages.inventory_data_types.create.01'))
        ...
        <div class="col-xl-12">
            <div class="card shadow-sm">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-success fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">{{ __('messages.inventory_data_types.create.02') }}</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar"></div>
                </div>
                <div class="card-body">
                    <form action="{{ route('InventoryDataTypes.Store') }}" id="createForm" enctype="multipart/form-data" method="POST" autocomplete="off">
                        @csrf
                        <div class="row">
                            <div class="col-12">
                                <label for="input-name" class="required form-label">{{ __('messages.inventory_data_types.form.01') }}</label>
                                <input type="text" name="input-name" id="input-name" class="form-control @error('input-name') is-invalid error-input @enderror" placeholder="{{ __('messages.inventory_data_types.form.01') }} {{ __('messages.forms.01') }}" maxlength="1000" value="{{ old('input-name') }}"/>
                        ...
                        <div class="row mt-6">
                            <div class="col-6">
                                ...
                                <select class="form-select @error('input-is_active') is-invalid error-input @enderror" name="input-is_active" id="input-is_active" data-control="select2" data-placeholder="{{ __('messages.forms.06') }}" data-hide-search="true">
                                    <option></option>
                                    <option value="1" {{ old('input-is_active') == '1' ? 'selected' : '' }}>{{ __('messages.inventory_data_types.form.02.01') }}</option>
                                    <option value="0" {{ old('input-is_active') == '0' ? 'selected' : '' }}>{{ __('messages.inventory_data_types.form.02.02') }}</option>
                                </select>
                            ...
                            <div class="col-6">
                                ...
                                <select class="form-select @error('input-is_data_special') is-invalid error-input @enderror" name="input-is_data_special" id="input-is_data_special" data-control="select2" data-placeholder="{{ __('messages.forms.06') }}" data-hide-search="true">
                                    <option></option>
                                    <option value="1" {{ old('input-is_data_special') == '1' ? 'selected' : '' }}>{{ __('messages.inventory_data_types.form.03.01') }}</option>
                                    <option value="0" {{ old('input-is_data_special') == '0' ? 'selected' : '' }}>{{ __('messages.inventory_data_types.form.03.02') }}</option>
                                </select>
